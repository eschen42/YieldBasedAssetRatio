...
For example, total real^[Throughout this paper, all returns, growth, and rates of growth (i.e., CAGR) are expressed in "real" (i.e., inflation-adjusted) terms.] returns for the Standard & Poor's 500 stock index (SP500) were painfully low over 20-year intervals starting in 1912, 1929, and 1961, and they were barely competitive with bonds over 1988–2008. One may argue that September 1961 started the worst 20-year interval since 1911 because, between 1969 and 1981, real returns from both SP500 and 10-year US Treasury bonds (GS10) were so minimal that SP500 had real growth of only 7% over the entire twenty years starting March of 1962 (compound annual growth rate, CAGR: 0.34%), while GS10 lost 33% of its real value (CAGR: –1.98%); portfolios having a fixed-percentage stock allocation would have performed between these rates of return.
...
> *In the ordinary common stock ... the margin of safety lies in an expected earning power considerably above the going rate for bonds.*^[Graham, 2003. *Op. cit.*, pp. 512--524, italics added. For example, Graham would assign a "75% margin of safety" to a stock with expected earning power of 7% at a time when the current yield for bonds is 4%.]
...
Inspection of Shiller's data set reveals that, since 1911, earnings yields for stocks and (nominal) interest for 10-year government bonds have been weak estimators of their respective 10-year real returns:
...
- Ordinarily, do not trade stock otherwise.^[Implicit in the formula is the assumption that distributions taken to replenish low-volatility assets (which are earmarked for and consumed by ongoing spending) would be taken from the lower-yielding asset in the portfolio. This might lead to the sale of stock (likely at a profit).]
...
Returns from the YBAR were much lower than returns from two fixed-percentage stock allocations for 20-year intervals starting between 1976 and 1989, but this would not have threatened funding of a retirement because 20-year returns were very high at that time, even for the YBAR.
...
Figure 8 shows that the YBAR would be beneficial for intervals of length greater than ten years. 
...
- For this data set, there are only about five times that the YBAR can be subjected to performance tests under observed conditions where relatively reduced stock yield adversely affected stocks' intermediate return (1912, 1929, 1961--1963, 1988, and 1998).
  - No attempt was made to model the amount or sequence of future returns. The lessons offered by the past are limited, so it is up to the reader to decide whether the analysis presented here is sufficiently thorough.
...
because 30-years may be fewer than one might expect their retirement to last but may be more than one can wait before needing access to some of the invested principal, he also compared those results to results for intervals of 1--60 years.^[Pfau, Wade D., 2011. *Long-Term Investors and Valuation-Based Asset Allocation.* Available at SSRN: *https://dx.doi.org/10.2139/ssrn.2544636*]
